...
module.exports = {
  ...
  /**
   * 获取项目的带路径名字
   * http://gitlab-ci-token:4xuDNC7KTnzyPs9J7Rac@gitlab.omcoin-inc.com/omfe/omt/assets-system-test.git
   * git@gitlab.omcoin-inc.com:omfe/omt/assets-system.git
   */
  getProjectPath() {
    if (process.env.ASSETS_PROJECT_PATH) {
      return process.env.ASSETS_PROJECT_PATH;
    }
    // let remoteUrl = `omfe/${pkg.name}`;
    // try {
    ...
    return remoteUrl.replace(/.*omcoin-inc\.com(.*)\.git/, '$1').replace(/^[:/]/, '');
  }
};
